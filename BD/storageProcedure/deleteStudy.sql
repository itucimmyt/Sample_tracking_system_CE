...
drop procedure if exists sp_delete_study;
...
begin
	...
	start transaction;
		...
		from `sampletrackingm`.`st_lab_study`
		...
		elseif exists(select `labstudyid` from `sampletrackingm`.`st_sample_detail`
			...
			delete I
			from `sampletrackingm`.`st_sampleid` I
			inner join `sampletrackingm`.`st_sample_detail` D ON
			...
			delete C
			from `sampletrackingm`.`st_sample_det_result` C
			inner join `sampletrackingm`.`st_sample_detail` D on
			      D.`studysampleid` = C.`studysampleid`
			  and D.`labstudyid` = id_study;

			delete R
			from `sampletrackingm`.`st_constants_cornell_report` R
			inner join `sampletrackingm`.`st_lab_study` D on
			      D.`labstudyid` = R.`labstudyid`
			  and D.`labstudyid` = id_study;

			delete from `sampletrackingm`.`st_sample_detail`
			where `labstudyid` = id_study;

			delete from `sampletrackingm`.`st_lab_study`
			where `labstudyid` = id_study;
